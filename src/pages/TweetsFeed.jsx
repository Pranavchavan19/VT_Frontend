...
import { useSelector, useDispatch } from "react-redux";
import { getAllTweets, deleteTweet, editTweet } from "../Store/Slices/tweetSlice";
import { Like, DeleteConfirmation, Edit } from "../components";
import { HiOutlineDotsVertical } from "../components/icons";
...
function TweetsFeed() {
    ...
    const handleEditTweet = (tweetId, editedContent) => {
        dispatch(editTweet({ tweetId, content: editedContent }));
        setEditState({ editing: false, editedContent: "", isOpen: false, delete: false, tweetId: null });
    };
    ...
    return (
        <div className="text-white w-full flex flex-col gap-3">
            {tweets && tweets.length > 0 ? (
                tweets.map((tweet) => (
                    <div key={tweet._id} className="flex gap-3 border-b border-slate-600 p-3">
                        <img
                        ...
                        <div className="w-full flex flex-col gap-2">
                            <div className="flex justify-between items-center">
                                <h2>{tweet.owner?.username}</h2>
                                <span>{timeAgo(tweet.createdAt)}</span>
                            </div>
                            {editState.editing && editState.tweetId === tweet._id ? (
                                <Edit
                                    initialContent={editState.editedContent}
                                    onCancel={() => setEditState({ editing: false, isOpen: false })}
                                    onSave={(editedContent) => handleEditTweet(tweet._id, editedContent)}
                                />
                            ) : (
                                <p>{tweet.content}</p>
                            ...
                            {authUsername === tweet.owner?.username && (
                                <div className="relative">
                                    ...
                                    />
                                    {editState.isOpen && editState.tweetId === tweet._id && (
                                        <div className="absolute right-0 bg-gray-700 p-2 rounded">
                                            <div
                                                onClick={() =>
                                                    setEditState({ editing: true, isOpen: false })
                                                }
                                            >
                                                Edit
                                            </div>
                                            <div
                                                onClick={() =>
                                                    setEditState({ delete: true, isOpen: false })
                                                }
                                            >
                                                Delete
                                            </div>
