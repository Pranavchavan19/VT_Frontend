...
function TweetsList() {
    const dispatch = useDispatch();
    const tweets = useSelector((state) => state.tweet?.tweets || []); // Adjust based on your state structure
    const avatar2 = useSelector((state) => state.user?.profileData?.avatar.url);
    const authUsername = useSelector((state) => state.auth?.userData?.username);
    ...
    return (
        <div className="w-full">
            {tweets.map((tweet) => (
                <div
                    key={tweet.id}
                    className="text-white w-full flex justify-start items-center sm:gap-5 gap-3 border-b border-slate-600 p-3 sm:p-5"
                >
                    <div className="w-10">
                        <img
                    ...
                    <div className="w-full flex flex-col gap-1 relative">
                        ...
                        {/* Display tweet content or edit form */}
                        {editState.editing && editState.tweetId === tweet.id ? (
                            <Edit
                                initialContent={editState.editedContent}
                                onCancel={() =>
                                    setEditState((prevState) => ({
                                ...
                                onSave={(editedContent) =>
                                    handleEditTweet(tweet.id, editedContent)
                                }
                            />
                        ) : (
                            <p>{tweet.content}</p>
                        ...
                        {authUsername === tweet.username && (
                            <>
                                ...
                                {editState.isOpen && editState.tweetId === tweet.id && (
                                    <div className="border bg-[#222222] text-lg border-slate-600 absolute text-center right-5 rounded-xl">
                                        <ul>
                                            <li
                                                className="hover:opacity-50 px-5 cursor-pointer border-b border-slate-600"
                        ...
                        {/* Delete Confirmation */}
                        {editState.delete && editState.tweetId === tweet.id && (
                            <DeleteConfirmation
                                tweet={true}
                                onCancel={() =>
                                    setEditState((prevState) => ({
