...
function HomePage() {
    ...
    const handleVideoClick = (videoId) => {
        setLoadingVideo((prev) => ({
            ...prev,
            [videoId]: true, // Mark video as loading when clicked
        }));
    };
    ...
    const handleCanPlay = (videoId) => {
        setLoadingVideo((prev) => ({
            ...prev,
            [videoId]: false, // Video is ready to play
        }));
    };
                    ...
                    {videos?.map((video) => (
                        <div key={video._id} className="video-container">
                            ...
                            <VideoList
                                ...
                                channelName={video.ownerDetails.username}
                                videoId={video._id}
                                onClick={() => handleVideoClick(video._id)}  // Mark video as clicked
                            />

                            {/* Loading Spinner or Placeholder */}
                            {loadingVideo[video._id] && (
                                <div className="loading-spinner">
                                    <Spinner width={10} /> {/* You can adjust the width */}
                                    <p>Loading Video...</p>
                                </div>
                            )}
                            ...
                            <video
                                ...
                                controls
                                style={{
                                    display: loadingVideo[video._id] ? "none" : "block", // Hide video while it's loading
                                    opacity: loadingVideo[video._id] ? 0 : 1, // Fade in the video once it's ready
                                    transition: "opacity 0.5s ease", // Smooth fade transition
                                }}
                                onCanPlay={() => handleCanPlay(video._id)} // Mark video as ready when it can play
                            >
                                <source src={video.videoUrl} type="video/mp4" />
                                Your browser does not support the video tag.
                            </video>
